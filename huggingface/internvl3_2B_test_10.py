...
prompts = [
        '任务指令是将包裹二维码朝上放置到扫码台扫码，然后放到传送带上。基于这张图，你应该处理哪个>包>裹，如何处理',
        '包裹、扫码区和传送带的具体位置和特征是什么？它们是如何排列的',
        '指令是“将所有包裹的二维码方向朝上进行扫码，然后放到右侧传送带上”，你会如何理解并执行这个>任>务',
        '任务是药盒的拣选、扫码、入库，当前机器人应该执行扫码操作',
        '描述机器人手中的药盒状态',
        '描述图像中两个机器人手臂的位置和姿态。它们目前处于什么状态',
        '如果指令是“从左侧药箱中拣选一个药盒，扫描其条形码后放入右侧入库箱”，你会如何理解并执行这>个>任务',
        '描述图像中两个机器人手臂的位置和姿态。它们目前处于什么状态？',
        '描述图像中包裹的放置情况',
        '你是人形机器人,你的任务是面前所有包裹二维码朝上滑到你右侧的传送带上，基于当前图像展现的情况>，你会先处理哪个包裹？',
        '你是人形机器人，如果接到指令“将所有包裹的二维码标签翻转朝上，然后挪到右侧传送带上“，基于>当>前图像展现的情况，你会如何操作？',
